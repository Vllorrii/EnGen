...
bool ColorShader::Initialize(ID3D11Device* device) {
	...
	result = D3DCompileFromFile(L"FX/color.vs", NULL, NULL, "ColorVertexShader", "vs_4_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &vertexShader, &errorMsg);
	if (FAILED(result)) {
		// If an error message was created, show it in a message box, else the file does not exist
		if (errorMsg) {
			MessageBox(NULL, (LPCWSTR)errorMsg->GetBufferPointer(), L"Shader error", MB_OK);
		} else {
	...
	result = D3DCompileFromFile(L"FX/color.ps", NULL, NULL, "ColorPixelShader", "ps_4_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &pixelShader, &errorMsg);
	if (FAILED(result)) {
		// If an error message was created, show it in a message box, else the file does not exist
		if (errorMsg) {
			MessageBox(NULL, (LPCWSTR)errorMsg->GetBufferPointer(), L"Shader error", MB_OK);
